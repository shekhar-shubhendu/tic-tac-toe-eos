   ...
   public:
      ...
      struct game {
         ...
         void initialize_board() {
            ...
            board[normalizeIndex(result.hash[0] * board_width + result.hash[1])] = 1;
            board[normalizeIndex(result.hash[2] * board_width + result.hash[3])] = 2;
            for(auto& item : board){
                  eosio::print((uint32_t)item);
            }
         }

         uint8_t normalizeIndex(uint8_t num){
               while(num > (board_width * board_height)-1){
                     uint8_t sum = 0;
                     for(uint8_t i = num; i>0; i=i/10){
                           sum += i%10;
                     }
                     num = sum;
               }
